...
function usePrefetchBalances() {
  ...
  useCurrencyBalances(account ?? undefined, tokenList !== prefetchedTokenList ? tokenList : undefined)
}
...
function useAreBalancesLoaded(): boolean {
  ...
  const balances = useCurrencyBalances(account ?? undefined, currencies).filter(Boolean)
